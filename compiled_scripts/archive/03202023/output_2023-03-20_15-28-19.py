...
def duplicate_df_for_shared_services(df):
    ...
    if df_shared_dups.loc[df_shared_dups['ALLOCATION_KEY'].isnull()].shape[0] > 0:
        print("The first 15 rows where the ALLOCATION_KEY is null:")
        print(df_shared_dups.loc[df_shared_dups['ALLOCATION_KEY'].isnull()].head(15))
...
def join_allocation_to_df(df, df_allocation):
    ...
    print("The first 15 rows of df_sum at the start:")
    print(df_sum.head(15).to_string(index=False))
    # print the rows where vendor is "Calendly LLC"
    print("The first 15 rows of df_sum where the Vendor is Calendly LLC:")
    print(df_sum.loc[df_sum['Vendor']=='Calendly LLC'].head(100).to_string(index=False))
    ...
    print("The first 15 rows of df where the Vendor is Calendly LLC:")
    print(df.loc[df['Vendor']=='Calendly LLC'].head(100).to_string(index=False))
    ...
    print("number of rows where Vendor is # NO VENDOR and the ALLOCATION is 0: " + str(df.loc[(df['Vendor']=='# NO VENDOR') & (df['ALLOCATION']==0)].shape[0]))
    # print the number of rows where ALLOCATION is 0
    print("number of rows where ALLOCATION is 0: " + str(df.loc[df['ALLOCATION']==0].shape[0]))
    print("allocating .5 to # NO VENDOR for CARE and CONNECT")
    # print the number of rows where Vendor is "# NO VENDOR" and the ALLOCATION is 0 and the SOURCE is not "FTE_AND_OPEN_ROLES"
    print("number of rows where Vendor is # NO VENDOR and the ALLOCATION is 0 and the SOURCE is not FTE_AND_OPEN_ROLES: " + str(df.loc[(df['Vendor']=='# NO VENDOR') & (df['ALLOCATION']==0) & (df['EXPENSE_BUCKET_2']!='FTE_AND_OPEN_ROLES')].shape[0]))
    ...
    print("after adjusting # NO VENDOR, the number of rows where ALLOCATION is 0 is: " + str(df.loc[df['ALLOCATION']==0].shape[0]))
    ...
    print("The unique values of BU are: " + str(df_missing_spend['BU'].unique()))
    ...
    if df_missing_spend.shape[0] > 0:
        ...
        print("Before the fix, the total spend is: " + str(df['ALLOCATED_AMOUNT'].sum()))
        print("There are rows where spend is missing:")
        print(df_missing_spend.to_string())
        # print the count of unique BUs for each Vendor and EXPENSE_BUCKET_2 where AMOUNT is not null and the AMOUNT is not 0
        print("The count of unique BUs for each Vendor and EXPENSE_BUCKET_2 where AMOUNT is not null and the AMOUNT is not 0:")
        print(df_missing_spend.groupby(['Vendor', 'EXPENSE_BUCKET_2'])['BU'].nunique().to_string())
        ...
        print("After the fix, the total spend is: " + str(df['ALLOCATED_AMOUNT'].sum()))
        
    else:
        print("There are no rows with missing spend")
    # remove rows where the ALLOCATED_AMOUNT is null or 0
    df = df.loc[~df['ALLOCATED_AMOUNT'].isnull()]
    df = df.loc[df['ALLOCATED_AMOUNT'] != 0]
    print("PART 3 : The first 15 rows of df where the Vendor is Calendly LLC:")
    print(df.loc[df['Vendor']=='Calendly LLC'].head(100).to_string(index=False))
    ...
    if df.loc[(df['ALLO_TYPE']=='SHARED SERVICES') & (df['ALLOCATION']==0)].shape[0] > 0:
        print("The first 15 rows where ALLO_TYPE is SHARED SERVICES and the ALLOCATION is 0:")
        print(df.loc[(df['ALLO_TYPE']=='SHARED SERVICES') & (df['ALLOCATION']==0)].head(15))
    else:
        print("There are no rows where ALLO_TYPE is SHARED SERVICES and the ALLOCATION is 0")
        ...
        if df.loc[(df['ALLO_TYPE']=='SHARED SERVICES') & (df['ALLOCATION']==1)].shape[0] > 0:
            print("The first 15 rows where ALLO_TYPE is SHARED SERVICES and the ALLOCATION is 1:")
            print(df.loc[(df['ALLO_TYPE']=='SHARED SERVICES') & (df['ALLOCATION']==1)].head(15))
        else:
            print("There are no rows where ALLO_TYPE is SHARED SERVICES and the ALLOCATION is 1")
        # print the sum of ALLOCATED_AMOUNT for each EXPENSE_BUCKET_2
        print("The sum of ALLOCATED_AMOUNT for each EXPENSE_BUCKET_2:")
        print(df.groupby('EXPENSE_BUCKET_2')['ALLOCATED_AMOUNT'].sum().to_string())
    
    print("PART 4 : The first 15 rows of df where the Vendor is Calendly LLC:")
    print(df.loc[df['Vendor']=='Calendly LLC'].head(100).to_string(index=False))
    # print the head of df
    print("The values after allocating")
    ...
    try:
        ...
        print("The first 15 rows of df_sum:")
        print(df_sum.head(15).to_string())
    
    except:
        print("There was an error merging df_sum and df.groupby(['EXPENSE_BUCKET_2', 'Vendor'])['ALLOCATED_AMOUNT'].sum().reset_index()")
        print("df_sum:")
        print(df_sum.head(15))
        print("df.groupby(['EXPENSE_BUCKET_2', 'Vendor'])['ALLOCATED_AMOUNT'].sum().reset_index():")
        print(df.groupby(['EXPENSE_BUCKET_2', 'Vendor'])['ALLOCATED_AMOUNT'].sum().reset_index().head(15))
    ...
    print("The sum of ALLOCATION for each Vendor, BU, Quarter, Year, and EXPENSE_BUCKET_2:")
    df_allo_total = df_checking.groupby(['Vendor', 'QUARTER', 'YEAR', 'EXPENSE_BUCKET_2'])['ALLOCATION'].sum()
    print("The number of rows for each Vendor, Quarter, Year, and EXPENSE_BUCKET_2:")
    ...
    print("The sum of ALLOCATION for each Vendor, BU, Quarter, Year, and EXPENSE_BUCKET_2 divided by the number of rows for each Vendor, Quarter, Year, and EXPENSE_BUCKET_2:")
    print(df_allo_check.to_string())
    return df
