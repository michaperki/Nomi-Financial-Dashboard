...
def join_allocation_to_df(df, df_allocation):
    ...
    logging.debug(df.loc[(df['Vendor']=='# NO VENDOR') & (df['ALLOCATION']==0)].to_string())
    ...
    try:
        ...
        logging.debug(df.groupby(['EXPENSE_BUCKET_2', 'Vendor'])['ALLOCATED_AMOUNT'].sum().reset_index().head(15))
...
def validation_complete(val_dict, df):
    ...
    logging.debug("type: ", type(ACTUALS_SUM_DETAIL_C2))
    logging.debug(ACTUALS_SUM_DETAIL_C0.to_string())
    logging.debug("type: ", type(ACTUALS_SUM_DETAIL_C0)) 
...
def format_headcounts(df):
    ...
    print("Number of rows where MONTH is 99: ", len(df.loc[df['MONTH']=="99"]))
    df['ALLOCATED_AMOUNT'] = df['AMOUNT'] * df['ALLOCATION']
    ...
    if df_len != len(df):
        print("Removed rows with 0 allocated amount")
        print(df_len - len(df), "rows removed")
