...
### QUARTERLY FILE PATHS ###
PATH = "data/2023/quarterly/"
FTE_QUARTERLY_DICT = {
    'DOMO_FILE': "FTE Projections (QUARTERLY)",
    'FILE_PATH': PATH + "FTE Projections (QUARTERLY).csv",
    'SHEET_NAME': "Roster"
}
ORM_QUARTERLY_DICT = {
    'DOMO_FILE': "Open Roles Projections (QUARTERLY)",
    'FILE_PATH': PATH + "Open Roles Projections (QUARTERLY).csv",
    'SHEET_NAME': "Open Roles"
}
FTC_QUARTERLY_DICT = {
    'DOMO_FILE': "FTC Projections (QUARTERLY)",
    'FILE_PATH': PATH + "FTC Projections (QUARTERLY).csv",
    'SHEET_NAME': "FTC"
}
PRO_SERV_QUARTERLY_DICT = {
    'DOMO_FILE': "Pro Services Projections (QUARTERLY)",
    'FILE_PATH': PATH + "Pro Services Projections (QUARTERLY).csv",
    'SHEET_NAME': "Pro Services"
}
SOFTWARE_QUARTERLY_DICT = {
    'DOMO_FILE': "Software Projections (QUARTERLY)",
    'FILE_PATH': PATH + "Software Projections (QUARTERLY).csv",
    'SHEET_NAME': "Software"
}
ACTUALS_QUARTERLY_DICT = {
    'DOMO_FILE': "Actuals (Quarterly)",
    'FILE_PATH': PATH + "Actuals (Quarterly).csv",
}
...
def format_final_df(df, ACTUAL_FILE_DATE):
    ...
    try:
        ...
        logging.debug("Unable to set CASH_VIEW for projections, possibly no CASH_VIEW column in the data frame")
...
def get_quarters_from_file_name(df):
    ...
    if df["Y-budget"].isnull().values.any():
        print("some file names did not include a year value, assigning 2023")
    ...
    # These print statements are for debugging purposes only
    # print the unique values in the Q-budget column
    # print("Unique values in Q-budget column: ", df["Q-budget"].unique())
    # print the unique values in the Y-budget column
    # print("Unique values in Y-budget column: ", df["Y-budget"].unique())
    return df
...
def calculate_qm_diff(df_main, df_quarterly):
    ...
    df_monthly_diff = df_monthly_diff[['YEAR', 'QUARTER', 'MONTH', 'Vendor', 'BU', 'Function', 'SAL_BONUS', 'IS Grouping', 'EXPENSE_BUCKET', 'ALLO_TYPE', 'VALUE_MONTHLY']]
    df_quarterly_diff = df_quarterly_diff[['YEAR', 'QUARTER', 'MONTH', 'Vendor', 'BU', 'Function', 'SAL_BONUS', 'IS Grouping', 'EXPENSE_BUCKET', 'ALLO_TYPE', 'VALUE_QUARTERLY']]
    # join the data frames using a full outer join
    df_qm_diff = pd.merge(df_monthly_diff, df_quarterly_diff, how='outer', on=['YEAR', 'QUARTER', 'MONTH', 'Vendor', 'BU', 'Function', 'SAL_BONUS', 'IS Grouping', 'EXPENSE_BUCKET', 'ALLO_TYPE'])
    ...
    df_qm_diff = df_qm_diff.groupby(['YEAR', 'QUARTER', 'MONTH', 'BU', 'EXPENSE_BUCKET', 'ALLO_TYPE', 'Function', 'SAL_BONUS', 'IS Grouping', 'Vendor']).agg({'QM_DIFF': 'sum', 'VALUE_MONTHLY': 'sum', 'VALUE_QUARTERLY': 'sum'}).reset_index()
...
def main():
    ...
    if QM_DIFF_CONTROLLER:
        df_qm_diff = calculate_qm_diff(df_fte, df_ftc, df_pro_serv, df_software, df_actuals, ACTUAL_FILE_DATE, df_name_map)
