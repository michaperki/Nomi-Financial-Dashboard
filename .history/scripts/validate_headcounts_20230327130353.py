...
def validate_headcounts(df_fte, df_orm, df_headcounts):
    ...
    logging.info("\n\nHeadcounts")
    # get the sum of ALLOCATION in df_headcounts for each year
    hc_2019 = df_headcounts[df_headcounts['Year'] == 2019]['ALLOCATION'].sum()
    logging.info(f"Number of 2019 values in the 'ALLOCATION' column: {hc_2019}")
    hc_2020 = df_headcounts[df_headcounts['Year'] == 2020]['ALLOCATION'].sum()
    logging.info(f"Number of 2020 values in the 'ALLOCATION' column: {hc_2020}")
    hc_2021 = df_headcounts[df_headcounts['Year'] == 2021]['ALLOCATION'].sum()
    logging.info(f"Number of 2021 values in the 'ALLOCATION' column: {hc_2021}")
    hc_2022 = df_headcounts[df_headcounts['Year'] == 2022]['ALLOCATION'].sum()
    logging.info(f"Number of 2022 values in the 'ALLOCATION' column: {hc_2022}")
    hc_2023 = df_headcounts[df_headcounts['Year'] == 2023]['ALLOCATION'].sum()
