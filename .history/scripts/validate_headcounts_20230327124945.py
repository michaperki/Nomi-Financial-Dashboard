...
def validate_headcounts(df_fte, df_orm, df_headcounts):
    ...
    hc_no_depart = len(df[df['Departure Date'].isna()])
    logging.info(f"Number of null values in the 'Departure Date' column: {hc_no_depart}")
    hc_depart_2023 = len(df[df['Departure Date'].str.contains('2023')])
    logging.info(f"Number of 2023 values in the 'Departure Date' column: {hc_depart_2023}")
    hc_depart_prior = hc_total - hc_no_depart - hc_depart_2023
    logging.info(f"Number of prior values in the 'Departure Date' column: {hc_depart_prior}")
