    ...
    if df_allo_total_by_Q_sum.shape[0] > 0:
        ...
        if df_allo_total_missing.shape[0] > 0:
            ...
            if df[df['SOURCE'] == 'FTE_AND_OPEN_ROLES'].shape[0] > 0:
                ...
                print("df_allo_total_missing")
                print(df_allo_total_missing.to_string(index=False))
                for index, row in df_allo_total_missing.iterrows():
                    df.loc[(df['Vendor'] == row['Vendor']) & (df['QUARTER'].isin(row['QUARTER'])) & (df['YEAR'] == row['YEAR']) & (df['SOURCE'] == 'FTE_AND_OPEN_ROLES') & (df['BU'].isin(['Care','Connect'])), 'ALLOCATION'] = .5
                    df.loc[(df['Vendor'] == row['Vendor']) & (df['QUARTER'].isin(row['QUARTER'])) & (df['YEAR'] == row['YEAR']) & (df['SOURCE'] == 'FTE_AND_OPEN_ROLES') & (df['BU'].isin(['Insights','Network'])), 'ALLOCATION'] = 0   
                # store the sum of the ALLOCATION column in df where the SOURCE is "FTE_AND_OPEN_ROLES" in a variable
                ...
                if df_allo_total_by_Q_sum.shape[0] > 0:
                    ...
                    # if there are rows in the data frame, print the rows
                    if df_allo_total_missing.shape[0] > 0:
                        logging.warning("There are rows where the ALLOCATION column does not equal 1 and the SOURCE is FTE_AND_OPEN_ROLES")
                        logging.warning(df_allo_total_missing.to_string(index=False))
                    else:
                        logging.warning("No rows where the ALLOCATION column does not equal 1 and the SOURCE is FTE_AND_OPEN_ROLES")
        else:
