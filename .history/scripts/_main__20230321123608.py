...
def main():
    ...
    if QUARTERLY_CONTROLLER:
        df_fte = get_data(FTE_QUARTERLY_DICT)
        df_orm = get_data(ORM_QUARTERLY_DICT)
        df_ftc = get_data(FTC_QUARTERLY_DICT)
        df_pro_serv = get_data(PRO_SERV_QUARTERLY_DICT)
        df_software = get_data(SOFTWARE_QUARTERLY_DICT)
        df_actuals = get_data(ACTUALS_QUARTERLY_DICT)
        df_name_map = get_data(NAME_MAP_DICT)
        df_fte_allocation = get_data(FTE_ALLOCATION_DICT)
        df_ftc_allocation = get_data(FTC_ALLOCATION_DICT)
        df_pro_serv_allocation = get_data(PRO_SERV_ALLOCATION_DICT)
        df_software_allocation = get_data(SOFTWARE_ALLOCATION_DICT)
