...
def complete_data_import(df):
    ...
    if '_FILE_NAME_' in df.columns:
        logging.INFO("the file name is " + df['_FILE_NAME_'].iloc[0])
    else:
        logging.INFO("the file name is not in the data frame, probably the NAME MAP")
    
    # print the dimensions of the data frame
    logging.INFO("the dimensions of the data frame are " + str(df.shape[0]) + " rows and " + str(df.shape[1]) + " columns")
    print_columns(df)
# END SCRIPT