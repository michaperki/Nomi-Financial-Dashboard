...
def clean_allocation_sheet(df, name_map):
    ...
    if df.loc[(df['QUARTER'].isnull()) | (df['BU'].isnull()) | (df['Vendor'].isnull()) | (df['YEAR'].isnull()) | (df['SOURCE'].isnull())].shape[0] > 0:
        print("There are null values in the QUARTER, BU, Vendor, YEAR, or SOURCE columns of the allocation sheet:")
        print(df.loc[(df['QUARTER'].isnull()) | (df['BU'].isnull()) | (df['Vendor'].isnull()) | (df['YEAR'].isnull()) | (df['SOURCE'].isnull())])
        print("")
    ...
    if df_allo_total_by_Q_sum.shape[0] > 0:
        ...
        print("Sum of the ALLOCATION column for each QUARTER, YEAR, Vendor, and SOURCE where ALLOCATION does not equal one:")
        print(df_allo_total_by_Q_sum.to_string(index=False))
        print("")
        # if there are rows in the data frame, print the rows
