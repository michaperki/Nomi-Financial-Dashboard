...
def main():
    ...
    if DELTA_CONTROLLER:
        df_actuals = get_data(ACTUALS_DICT)    
        df_software = get_data(SOFTWARE_DICT)
        df_ftc = get_data(FTC_DICT)
        df_pro_serv = get_data(PRO_SERV_DICT)
        df_fte = get_data(FTE_DICT)
        all_data = [df_actuals, df_software, df_ftc, df_pro_serv, df_fte]
        df = calculate_deltas(df_fte, df_ftc, df_pro_serv, df_software, df_actuals, ACTUAL_FILE_DATE, df_name_map)
