...
def compile_scripts():
    ...
    for file in file_order:
        ...
        with open(input_path, "r") as f:
            ...
            # Print the results
            print(f"{file}: {num_lines} lines, {num_imports} imports, {num_prints} prints, {num_loggings} loggings")
            results.append([file, num_lines, num_imports, num_prints, num_loggings])

    # convert the results to a data frame
    df = pd.DataFrame(results, columns=["file", "num_lines", "num_imports", "num_prints", "num_loggings"])

    # sort the data frame by the number of print statements
    df = df.sort_values(by="num_prints", ascending=False)
